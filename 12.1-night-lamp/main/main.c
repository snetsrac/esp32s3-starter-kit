 ...
 * Description: Use a photoresistor to control an LED.
*/
...
*/
static uint32_t calculate_duty(const uint32_t n) {
    assert(n >= 0);
    assert(n < DUTY_MAX);
...
/**
 * @brief Clamp duty ratio to within a resistance range.
 * 
 * @param resistance
*/
static void clamp_duty_ratio(uint32_t resistance) {
    float duty_ratio;

    if (resistance < R_MIN) {
        duty_ratio = 0.0f;
    } else if (resistance > R_MAX) {
        duty_ratio = 1.0f;
    } else {
        duty_ratio = (resistance - R_MIN) / (float)R_MAX;
    }
...
static void vTaskAdcLedControl(void *pvParameters) {
    ...
    // R = (10K * Vr/Vdd)/(1-Vr/Vdd) by voltage divider
    for (;;) {
            volts_ratio = adc1_get_raw(ADC_CHANNEL) / 4096.0f;
            resistance = (uint32_t)(10000 * volts_ratio / (1 - volts_ratio));

            duty_ratio = clamp_duty_ratio(resistance);
            duty = calculate_duty(255 * duty_ratio);
